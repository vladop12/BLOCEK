...
4. **ProcessingPanel** - Komponenta pre spracovanie vybraných súborov
5. **ResultsViewer** - Komponenta pre zobrazenie výsledkov spracovania
...
- `setup_ui(self)` - Nastavenie užívateľského rozhrania
- `browse_directory(self)` - Otvorenie dialógu na výber adresára
...
- `toggle_select_all(self)` - Prepínanie výberu všetkých súborov

### 4. ProcessingPanel (src/ui/processing_panel.py)
...
### 5. ResultsViewer (src/ui/results_viewer.py)
...
3. Keď používateľ klikne na tlačidlo "Spracovať" v **ProcessingPanel**, MainWindow získa vybrané súbory z **FileList** a spustí ich spracovanie.
4. Počas spracovania MainWindow aktualizuje progress bar a stav v **ProcessingPanel**.
5. Po dokončení spracovania MainWindow zobrazí výsledky v **ResultsViewer**.
6. Keď používateľ klikne na tlačidlo "Generovať XML" v **ProcessingPanel**, MainWindow spustí generovanie XML súborov.
...
5. Používateľ klikne na tlačidlo "Spracovať" v **ProcessingPanel**.
6. Aplikácia spracuje vybrané súbory a zobrazí výsledky v **ResultsViewer**.
7. Používateľ môže kliknúť na tlačidlo "Generovať XML" v **ProcessingPanel** pre generovanie XML súborov.
```

Teraz vytvoríme tento súbor:

```bash
New-Item -ItemType File -Path docs\ui_components.md -Value "# Dokumentácia UI komponentov projektu BLOCEK
...
- `__init__(self, parent, process_callback=None, generate_xml_callback=None)` - Inicializácia komponenty
- `setup_ui(self)` - Nastavenie užívateľského rozhrania
- `on_process_button_click(self)` - Callback metóda volaná po kliknutí
